...
export class UsersService {
  ...
  async findOrCreateProfile(supabaseUser: any) {
    ...
    return {
      ...
      gamification: this.getLevelProgress(user.totalXp ?? 0),
    };
  }
  ...
  getLevelProgress(totalXp: number) {
    ...
    const xpInsideLevel = totalXp - currentLevelXp;

    const xpRequiredThisLevel = nextLevelXp - currentLevelXp;

    const progressPercent =
      xpRequiredThisLevel <= 0
        ? 100
        : Number(((xpInsideLevel / xpRequiredThisLevel) * 100).toFixed(2));
