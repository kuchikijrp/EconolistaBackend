  ...
  async getDashboard() {
    ...
    let html = `
      ...
      <html lang="pt-BR">
        ...
        /* Barra de progresso com a cor #38bdf8 e listras */
        .barra-regressiva {
            height: 100%;
            /* Cria o padrão de listras usando gradiente linear */
            ...
            background-color: #38bdf8; /* Sua cor base */
            
            /* Duas animações: uma para esvaziar (5s) e outra para mover as listras */
            animation: 
                contagemRegressiva 5s linear forwards,
                moverListras 1s linear infinite;
        }

        /* Animação para esvaziar a barra de 100% a 0% */
        @keyframes contagemRegressiva {
            from { width: 100%; }
            to { width: 0%; }
        }

        /* Animação para fazer as listras se moverem */
        @keyframes moverListras {
    ...
    for (const log of dashboardLogs) {
      ...
      let msgHtml = '';
      let parsedMsg = log.message;

      if (typeof log.message === 'string') {
        try {
          const attempt = JSON.parse(log.message);
      ...
      if (typeof parsedMsg === 'object' && parsedMsg !== null) {
        msgHtml = `<pre class="json-block">${JSON.stringify(parsedMsg, null, 2)}</pre>`;
      } else {
      ...
      html += `
        <tr>
          ...
          <td class="context">[${log.method}]</td>
